...
- ⭐ **Carátulas** (extensión JEP-Hub, no existía en el original): un ítem puede ser una **carátula** — línea-título (ej. *ISLA 8 PUESTOS*) que agrupa productos reales (jerarquía de 1 nivel: `LineItem.tipo` PRODUCTO/CARATULA + `parentId` + `posicion`). Reglas:
  - La carátula no tiene referencia, precio ni acabados propios; su valor mostrado **siempre se deriva** de la suma de sus productos (la fila guarda montos en 0 → `sum(total de líneas) == subtotal` se mantiene).
  - Toda carátula debe tener **≥1 producto**; el subtotal de la cotización suma **solo productos**.
  - **PDF/impresión y firma del cliente:** la carátula se imprime como **una sola línea con la suma**; el desglose no se muestra al cliente. La página de impresión acepta `?detalle=1` (uso interno) para listar además los productos internos.
  - Vistas internas (detalle de cotización/pedido, correo de respaldo): muestran la carátula como fila-sección **con** su desglose.
  - **ERP ofimática:** la carátula **nunca** viaja como renglón de la CV (no existe en `MTMERCIA`); se insertan solo sus productos.
  - Al generar el pedido y al duplicar la cotización, la estructura (carátula + hijos) se copia completa.
- Campo `Observación`.
