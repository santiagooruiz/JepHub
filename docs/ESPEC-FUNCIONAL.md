...
- ⭐ **Carátulas y separadores** (extensión JEP-Hub, no existía en el original): un ítem puede ser una **carátula** — línea-título (ej. *ISLA 8 PUESTOS*) que agrupa productos reales (jerarquía de 1 nivel: `LineItem.tipo` PRODUCTO/CARATULA/SEPARADOR + `parentId` + `posicion`) — o un **separador** — línea de **solo texto** para seccionar la cotización (ej. *PISO 1*, *PISO 2*), sin montos ni hijos. Reglas:
  ...
  - **PDF/impresión y firma del cliente:** por defecto (vista **carátula**) la carátula se imprime como **una sola línea con la suma**; el desglose no se muestra al cliente (ni siquiera viaja en el payload). El separador **sí** se imprime como fila de sección. La página de impresión acepta `?detalle=1` (uso interno) para listar además los productos internos de cada carátula sin cambiar de vista. La vista **despiece** (`?vista=despiece`, botón "PDF despiece" en el detalle) es un modo aparte: lista **todos los productos en su propia línea**, sin carátulas ni separadores — para producción/fábrica.
...
- ⭐ **Acabados desde el ERP** (extensión JEP-Hub): al elegir un producto en el builder se consultan sus acabados en ofimática (`ZPROACA`/`ZACABADOS`: FORMICA, CANTO, HERRAJE, PAÑO…) y por cada uno aparece un **select buscable** con los materiales/colores disponibles (ítems `MTMERCIA` de esa familia + color `MT1CLAF`; en la lista se ve `CODIGO = DESCRIPCIO` y elegido muestra el color). Un acabado sin elegir queda **POR DEFINIR** (bloquea generar el pedido, como siempre). Las selecciones se guardan estructuradas (`LineItem.acabadosJson`) y el texto `acabados` que imprime el PDF/correo se **deriva** de ellas (`FORMICA: Formica blanco nevado [F8] · CANTO: POR DEFINIR`). Con el ERP fuera de línea el builder conserva el texto heredado del catálogo local.
  - **Acabados globales:** cuando 2+ productos de la cotización llevan la misma familia de acabado (ej. 3 productos con FORMICA), el builder muestra un panel con un select por familia que aplica la opción elegida a **todos** esos productos de una vez (luego se puede ajustar un producto puntual en su fila). El select muestra la opción común si todos coinciden, o "(varios)" si difieren; es una ayuda de captura, no se persiste nada adicional.
...
- ⭐ **Ítem ESPECIAL** (extensión JEP-Hub): en el select de producto, buscar "Especial" ofrece **`CODIGO-ESPECIAL`** — un producto por desarrollar, **sin precio** (el servidor fuerza 0), que solo captura **cantidad**, **descripción** y un **archivo opcional** (subido a MinIO vía `/api/files`, anclado al cliente; la URL queda en `LineItem.imagen`). Al guardar la cotización se crea automáticamente (idempotente por cotización) una **solicitud en Backlog Diseño** con el detalle y el archivo, y se **notifica** a los usuarios con rol Diseñador/Diseñador Comercial para que lo revisen. Mientras la cotización tenga ítems ESPECIAL **no se puede generar el pedido** (deben reemplazarse por la referencia definitiva que resuelva diseño).
- Campo `Observación`.
...
| `Solicitar planos/cambios` | Enviar a diseño (→ Backlog Diseño). Panel **"Solicitudes de plano comercial"** en la cotización: historial de versiones (v1, v2…), cada una con `Levantamiento` (archivos de referencia que sube el asesor: DWG/Sketchup/PDF/JPG/PPT-Word) y `Planos Comerciales` (archivo que sube diseño; al subirlo la versión se cierra). Notifica in-app + correo (`DESIGN_NOTIFY_EMAIL`) al equipo de diseño en cada versión nueva; pedir un cambio sobre una versión cerrada abre la siguiente versión encadenada. |
| `Solicitar ficha comercial` | Generar ficha comercial |
| `Cliente para facturación` | Asignar datos de facturación |
| `Descargar cotización en PDF` | PDF **carátula** (`/print/cotizacion/{id}`, por defecto): carátulas colapsadas (título + suma) — lo que ve el cliente |
| `Descargar cotización con despiece en PDF` | PDF **despiece** (`?vista=despiece`): todos los productos en su propia línea (referencia/precio/cant./total), **sin** filas de carátula ni de separador — para producción/fábrica |
